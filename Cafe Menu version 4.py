    ...
    while True:
        ...
        if food == "done":
            ...
            if not cart:
                ...
                remove_item = easygui.enterbox("Type the name of the item to remove:").strip().lower()
                if remove_item in cart:
                    cart.remove(remove_item)
                    easygui.msgbox(f"{remove_item.title()} was removed from your cart.")
                else:
                    easygui.msgbox(f"{remove_item.title()} is not in your cart.")
        elif food in food_menu or food in drink_menu:
            cart.append(food)
            easygui.msgbox(f"You added {food.title()} to your cart!")
        else:
